####################################################################################################################################################
## XCMAX4 can be used to detect the genetic association between a diallelic marker and a binary phenotype outcome (e.g., case-control status).  ###
## XCMAX4 will run the XCMAX4, FM01, FM02, FMs, and FMf  and reports the corresponding p-values. ####
## Arguments:                                                                                                                                   ###
## data: This is the data contains the phenotype, genotype and covariates information. Each row of data represents a different individual.      ###
## The first column of data records the phenotype information, with 1 representing a case status and 0 representing a control status.           ###
## The second column of data records the genotype information, with 0, 1 and 2 representing the number of risk allele.                          ###
## The remaining columns of data records all the covariate information and the variable name for gender must be "gender".                       ###
####################################################################################################################################################


XCMAX4 <- function(data){
  D=as.matrix(data["D"]) 
  G=as.matrix(data["genotype"])
  myvars <- names(data) %in% c("D","genotype")
  X=as.matrix(data[!myvars]) 
  ...
  ###compute FM01 and FM02
  fit1 <- glm(D ~ X + g01, family = binomial(link = "logit"))
  coef_01 <- summary(fit1)$coefficients
  p_fm01 <- coef_01[dim(coef_01)[1],dim(coef_01)[2]]
  fit2 <- glm(D ~ X + g02,family = binomial(link = "logit"))
  coef_02 <- summary(fit2)$coefficients
  p_fm02 <- coef_02[dim(coef_02)[1],dim(coef_02)[2]]
  
  ind_male <- data$gender==0
  D_m <- as.matrix(D[ind_male,])
  X_m <- X[ind_male,]
  genotype_m <- data$genotype[ind_male]
  fit_m <- glm(D_m ~ X_m+genotype_m,family = binomial())
  coef_m <- summary(fit_m)$coefficients
  z_m <- coef_m[dim(coef_m)[1],3] 
  
  D_f <- as.matrix(D[!ind_male,])
  X_f <- X[!ind_male,]
  genotype_f <- data$genotype[!ind_male]
  fit_f <- glm(D_f ~ X_f+genotype_f,family = binomial())
  ...
  # If you want to obtain the exact p value of XCMAX4,please uncomment the following two lines of code. 
  #library(mvtnorm)
  ...
  return(list("p-value for XCMAX4 (rhombus formual)"=p_rh,
              "p-value for FM01 (optimal for XCI-E)"=p_fm01,
              "p-value for FM02 (optimal for XCI-R)"=p_fm02,
              "p-value for FM_F (combined by Fisher's method)"=p_fisher,
              "p-value for FM_S (combined by Stouffer's method)"=p_stouffer)) 
}
